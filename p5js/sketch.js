...
function pos_to_color(x, y, x_n_cycles, y_n_cycles){
  let x_map = x/H*2*PI*x_n_cycles*y;
  let y_map = y/H*2*PI*y_n_cycles*x;
...
function draw_background(){
  ...
  let x_n_cycles = random(1, 20);
  let y_n_cycles = random(1, 20);
  for (let x=1; x <= W; x++ ){
    for (let y=1; y <= H; y++ ){
      col_g = pos_to_color(x + x_shift, y + y_shift, x_n_cycles, y_n_cycles);
      stroke(0, col_g, 0);
      strokeWeight(1);
      point(x, y);
    }
...
class Ball {
  ...
  f(sensor){
    return ceil(this.dna.genes[0]*sensor.a +
            (this.dna.genes[1])*sensor.b +
            (this.dna.genes[2])*sensor.c +
            (this.dna.genes[3])*sensor.d);
              //this.dna.genes[4]*t +
              //this.dna.genes[5];
  }
  ...
  get_dir(){
    let t = TIME/LIFE;
    let sensor = this.sensor_floor();

    let fx = this.f(sensor);
